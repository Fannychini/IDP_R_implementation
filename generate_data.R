...
generate_diverse_data <- function(n_patients_per_med, n_dispenses_per_patient) {
  ...
  for (med in medications) {
    for (i in 1:n_patients_per_med) {
      ...
      start_date <- as.Date("2018-01-01") + days(sample(0:10, 1))
      
      if (med == "Levodrax") { # = daily oral
        # regular monthly dispensing with small variations
        intervals <- c(NA, round(rnorm(n_dispenses_per_patient - 1, mean = 30, sd = 4)))
        ...
        intervals <- c(NA, round(rnorm(n_dispenses_per_patient - 1, mean = 21, sd = 3.5)))
        ...
        intervals <- c(NA, round(rnorm(n_dispenses_per_patient - 1, mean = 90, sd = 20)))
        ...
        quantity <- rep(30, n_dispenses_per_patient)
        strength_mg <- rep(125, n_dispenses_per_patient)
      }
